"""Graph Sync API endpoints — 图谱同步和查询（供主项目调用）."""
import logging
...
async def sync_graph(
    ...
    try:
        ...
        logger.info("[GraphSync] SUCCESS | job_id=%s kb=%s file=%s triples=%d nodes=%d | %s",
                    request.job_id, request.kb_name, request.file_name,
                    result.get("triples_count", 0), result.get("nodes_count", 0),
                    result.get("message", ""))
